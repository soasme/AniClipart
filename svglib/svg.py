...
class SVG:
    ...
    def save_png(self, file_path, background_color="white"):
        cairosvg.svg2png(bytestring=self.to_str(), write_to=file_path, background_color=background_color)

    def draw(self, fill=False, file_path=None, do_display=True, return_png=False, background_color=None,
        ...
        if return_png:
            if file_path is None:
                img_data = cairosvg.svg2png(bytestring=svg_str, background_color=background_color)
    ...
    def set_opacity(self, opacity):
        return self._apply_to_paths("set_opacity", opacity)
    ...
    def to_video(self, wrapper, color="black"):
        clips, svg_commands = [], []

        im = SVG([], viewbox=Bbox(1024)).draw(do_display=False, return_png=True)
    ...
    def animate(self, file_path=None, frame_duration=0.1, do_display=True):
        clips = self.to_video(lambda img: ImageClip(img).with_duration(frame_duration))
    ...
    def subdivide(self, edges, vertices):
        # divide the command that spands multiple triangles
        # edges: [num_edges, 2]
